<template>
  <div style="background-color: #f3f3f3;min-height: 100vh">
    <div class="noticeAssist" v-if="assistList.verify && assistList.verify.verifyMsg">
    ...
    <div class="wrap">
      ...
      <div class="footer">
        <div class="general">
          <span>推荐经销商</span>
          <van-cell-group class="groupAssist">
            <van-field disabled v-model="recommentName" placeholder="请输入身份证号码" />
          </van-cell-group>
        </div>
        <div class="general">
          <span>推荐经销商编号</span>
          <van-cell-group class="groupAssist">
            <van-field disabled v-model="recommentNum" placeholder="请输入身份证号码" />
          </van-cell-group>
        </div>
      </div>
    </div>

    <van-button disabled class="buttonSummitAssist" type="default" @click="submit" v-if="assistList.verify && assistList.verifyStatus != '1'">提交申请</van-button>
    <van-button class="buttonSummitAssist" type="default" @click="submit" v-else>提交申请</van-button>
...
<script>
  ...
  export default {
    ...
    computed:{
      ...mapState({
        'assistList':state => state.assist.assistList
      })
    },
    created(){
      this.$toast.loading({
      ...
      this.$store.dispatch('getAssistList')
        .then(res => {
          this.$toast.clear()
          if (this.assistList && this.assistList.verify){
    ...
    methods:{
      ...
      submit(){
        ...
        this.httpCli({
          url:config.URL_MEMBER_ASSIST_REGISTER_SUBMIT,
          data:data
        })
          .then(res => {
            this.$toast.clear()
