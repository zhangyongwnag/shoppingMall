<template>
  <div style="background-color: #f3f3f3;min-height: 100vh">
    <div class="wrap">
      <div class="general" v-if="$route.query.status">
        <span>原密码</span>
    ...
    <div class="register">
      <van-button round type="danger" class="login" @click="submit" >{{$route.query.status ? '修改密码' : '设置密码'}}</van-button>
    </div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods:{
      ...
      submit(){
        if (this.$route.query.status && !this.oldPassword){
          common.toast('请输入旧密码')
          return
        }
        ...
        this.httpCli({
          ...
          .then(res => {
            if (res.errorCode == 100){
              this.userInfo.memberLoginPasswordStatus = '1'
              localStorage.setItem('userInfo',JSON.stringify(this.userInfo))
              this.$route.query.status ? this.$toast.success('修改成功') : this.$toast.success('设置成功')
              setTimeout(() => {
                this.$router.back()
              },500)
            }
